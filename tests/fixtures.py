...
def make_imbalanced_binary_dataset(
    ...
    description = f"Imbalanced dataset: {n_samples} samples, {positive_fraction:.1%} positive"
...
def make_realistic_multiclass_dataset(
    ...
    if class_imbalance:
        ...
        if len(weights) < n_classes:
            ...
            weights.extend([remaining / (n_classes - len(weights))] * (n_classes - len(weights)))
    else:
        weights = None
    ...
    description = f"Multiclass dataset: {n_samples} samples, {n_classes} classes, {balance_str}"
...
def make_calibrated_binary_dataset(
    ...
    description = f"Calibrated dataset: {n_samples} samples, well-calibrated probabilities"
