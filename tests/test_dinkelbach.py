...
class TestDinkelbachMethod:
    ...
    def test_dinkelbach_basic_functionality(self):
        ...
        threshold, _ = dinkelbach_expected_fbeta_binary(y_prob, beta=1.0)
    ...
    def test_dinkelbach_edge_cases(self):
        ...
        # Another test case
        y_prob2 = np.array([0.2, 0.5, 0.8])
        threshold2, _ = dinkelbach_expected_fbeta_binary(y_prob2, beta=1.0)
    ...
    def test_dinkelbach_different_beta_values(self):
        ...
        for beta in [0.5, 1.0, 2.0]:
            threshold, _ = dinkelbach_expected_fbeta_binary(y_prob, beta=beta)
            assert isinstance(threshold, float)
            assert 0.0 <= threshold <= 1.0
    ...
    def test_dinkelbach_consistency(self):
        ...
        threshold1, _ = dinkelbach_expected_fbeta_binary(y_prob, beta=1.0)
        threshold2, _ = dinkelbach_expected_fbeta_binary(y_prob, beta=1.0)
