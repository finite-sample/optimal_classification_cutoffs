...
class TestDinkelbachComparisonSupport:
    ...
    def test_dinkelbach_tied_probabilities(self):
        ...
        for comparison in [">", ">="]:
            ...
            if comparison == ">":
                preds = np.array(pred_prob) > threshold
            else:
                preds = np.array(pred_prob) >= threshold
...
class TestMicroOptimizationDocumentation:
    ...
    def test_micro_smart_brute_warning(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            thresholds = _optimize_micro_averaged_thresholds(
...
class TestPropertyBased:
    ...
    def test_tie_semantics_consistency(self, n_samples, tie_prob):
        ...
        # Apply thresholds
        pred_excl = pred_prob > thresh_excl
        pred_incl = pred_prob >= thresh_incl
        ...
        if np.any(np.isclose(pred_prob, thresh_excl, atol=1e-10)):
            tied_indices = np.isclose(pred_prob, thresh_excl, atol=1e-10)
