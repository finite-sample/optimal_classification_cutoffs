...
import numpy as np
import pytest
from sklearn.metrics import f1_score as sklearn_f1, accuracy_score as sklearn_accuracy

from optimal_cutoffs import get_optimal_threshold, ThresholdOptimizer
from optimal_cutoffs.metrics import get_confusion_matrix, f1_score, accuracy_score
...
class TestRealisticBinaryOptimization:
    ...
    def test_imbalanced_binary_optimization(self):
        ...
        # All should be valid
        for name, thresh in [("F1", threshold_f1), ("Precision", threshold_precision), ("Recall", threshold_recall)]:
    ...
    def test_well_separated_vs_overlapping(self):
        ...
        assert f1_sep > f1_over + 0.1, f"Separated F1 {f1_sep:.3f} should be much better than overlapping F1 {f1_over:.3f}"
    ...
    def test_calibrated_data_expected_vs_empirical(self):
        ...
        threshold_emp = get_optimal_threshold(y_true, y_prob, metric="f1", mode="empirical")
        ...
        assert f1_emp > 0.4, f"Empirical F1 {f1_emp:.3f} should be decent on calibrated data"
        assert f1_exp > 0.4, f"Expected F1 {f1_exp:.3f} should be decent on calibrated data"
        
        # Expected F1 should be close to actual F1 for expected optimization
        assert abs(expected_f1 - f1_exp) < 0.1, f"Expected F1 {expected_f1:.3f} should match actual F1 {f1_exp:.3f}"
    
    def test_comparison_operators_realistic(self):
        """Test '>' vs '>=' comparison operators on realistic data."""
        y_true, y_prob = STANDARD_BINARY.y_true, STANDARD_BINARY.y_prob
        
        threshold_excl = get_optimal_threshold(y_true, y_prob, metric="f1", comparison=">")
        threshold_incl = get_optimal_threshold(y_true, y_prob, metric="f1", comparison=">=")
    ...
    def test_sample_weights_realistic(self):
        ...
        threshold_weighted = get_optimal_threshold(y_true, y_prob, metric="f1", sample_weight=weights)
        
        # Weighted threshold should be different (likely lower to catch more positives)
        assert abs(threshold_weighted - threshold_unweighted) > 0.01, "Weights should affect threshold"
...
class TestRealisticMulticlassOptimization:
    ...
    def test_standard_multiclass_optimization(self):
        ...
        for average in ["macro", "micro"]:
            ...
            assert all(0.0 <= t <= 1.0 for t in thresholds), "All thresholds should be valid"
            ...
            assert accuracy > 0.4, f"Multiclass accuracy {accuracy:.3f} should be reasonable for {average}"
    ...
    def test_imbalanced_multiclass_optimization(self):
        ...
        for thresholds, name in [(thresholds_macro, "macro"), (thresholds_micro, "micro")]:
            assert all(0.0 <= t <= 1.0 for t in thresholds), f"Invalid thresholds for {name} averaging"
    ...
    def test_multiclass_vs_binary_consistency(self):
        ...
        # Optimize as binary
        threshold_binary = get_optimal_threshold(
            y_true, y_prob_binary, metric="f1"
        )
...
class TestRealisticUtilityOptimization:
    ...
    def test_cost_sensitive_optimization(self):
        ...
        threshold_fn = get_optimal_threshold(y_true, y_prob, utility=utility_high_fn_cost)
        threshold_fp = get_optimal_threshold(y_true, y_prob, utility=utility_high_fp_cost)
    ...
    def test_bayes_vs_empirical_utility(self):
        ...
        # Empirical optimization
        threshold_emp = get_optimal_threshold(y_true, y_prob, utility=utility, mode="empirical")
        
        # Bayes optimization (doesn't need true labels)
        threshold_bayes = get_optimal_threshold(None, y_prob, utility=utility, mode="bayes")
...
def test_all_metrics_on_realistic_data(metric):
    ...
    assert 0.0 <= threshold <= 1.0, f"Metric {metric} produced invalid threshold {threshold}"
