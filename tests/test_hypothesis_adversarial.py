...
def integer_weights(min_value: int = 1, max_value: int = 3, min_size: int = 5, max_size: int = 50):
...
class TestWeightedEqualsExpanded:
    ...
    def test_integer_weights_exact_expansion(self, data, weights):
        ...
        if len(weights) != len(labels):
            weights = weights[:len(labels)] if len(weights) > len(labels) else np.pad(weights, (0, len(labels) - len(weights)), mode='wrap')
        ...
        for method in ['smart_brute', 'sort_scan']:
            ...
            try:
                ...
                threshold_expanded = get_optimal_threshold(
                    labels_expanded, probs_expanded,
                    metric=metric,
                    method=method
                )
                ...
                score_weighted = get_confusion_matrix(labels, probs, threshold_weighted, weights)
                score_expanded = get_confusion_matrix(labels_expanded, probs_expanded, threshold_expanded)
    ...
    def test_uniform_weights_match_unweighted(self, data, base_weight):
        ...
        threshold_weighted = get_optimal_threshold(
            labels, probs, metric='accuracy', method='smart_brute', sample_weight=uniform_weights
        )
...
class TestTieSemantics:
    ...
    @given(
        size=st.integers(15, 30),
        n_ties=st.integers(3, 8)
    )
...
class TestMicroAccuracyComparison:
    ...
    @given(
        data=multiclass_data(n_classes=3, min_size=30, max_size=60)
    )
...
class TestDinkelbachCalibration:
    ...
    @given(
        base_size=st.integers(50, 150),
        growth_factor=st.integers(2, 4)
    )
    @settings(max_examples=8, deadline=12000)
    def test_dinkelbach_monotone_behavior(self, base_size, growth_factor):
        ...
        if len(results) >= 2:
            ...
            assert all(f1 > 0.05 for f1 in f1_scores), "F1 scores too low for calibrated data"

    @given(
        size=st.integers(80, 200),
    ...
    def test_dinkelbach_vs_neighbors(self, size, noise_level):
        ...
        try:
            ...
            for neighbor_thresh in neighbors:
                if abs(neighbor_thresh - optimal_threshold) > 1e-6:  # Actually different
                    tp_n, tn_n, fp_n, fn_n = get_confusion_matrix(labels, probs, neighbor_thresh)
