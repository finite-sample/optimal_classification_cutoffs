...
def test_confusion_matrix_with_sample_weights():
    ...
    # Test with weights
    tp_w, tn_w, fp_w, fn_w = get_confusion_matrix(true_labs, pred_prob, threshold, sample_weight)
...
def test_multiclass_confusion_matrix_with_sample_weights():
    ...
    cms = get_multiclass_confusion_matrix(true_labs, pred_prob, thresholds, sample_weight)
...
def test_optimal_threshold_with_sample_weights():
    ...
    threshold_equal_weights = get_optimal_threshold(true_labs, pred_prob, "f1", sample_weight=equal_weights)
    ...
    threshold_weighted = get_optimal_threshold(true_labs, pred_prob, "f1", sample_weight=heavy_positive_weights)
...
def test_nested_cv_with_sample_weights():
    ...
    thresholds, scores = nested_cv_threshold_optimization(
        true_labs, pred_prob, metric="f1", inner_cv=3, outer_cv=2, sample_weight=sample_weight
    )
...
def test_sample_weights_piecewise_optimization():
    ...
    assert min_distance < 0.02, f"Threshold {threshold} not close to any candidate value. Candidates: {sorted(set(candidate_values))}"
