...
from .binary_optimization import metric_score, optimal_threshold_piecewise
from .metrics import multiclass_metric
from .types import ArrayLike, AveragingMethod, ComparisonOperator, OptimizationMethod, SampleWeightLike
from .validation import _validate_inputs


def validate_multiclass_input(true_labs: ArrayLike, pred_prob: ArrayLike) -> tuple[np.ndarray, np.ndarray]:
...
def multiclass_metric_score(
    ...
    if thresholds.shape[0] != n_classes:
        raise ValueError(f"thresholds must have shape ({n_classes},), got {thresholds.shape}")
    ...
    if metric == "f1":
        return f1_score(true_labs, predictions, average=average, sample_weight=sample_weight, zero_division=0)
    elif metric == "accuracy":
        return accuracy_score(true_labs, predictions, sample_weight=sample_weight)
    elif metric == "precision":
        return precision_score(true_labs, predictions, average=average, sample_weight=sample_weight, zero_division=0)
    elif metric == "recall":
        return recall_score(true_labs, predictions, average=average, sample_weight=sample_weight, zero_division=0)
    else:
