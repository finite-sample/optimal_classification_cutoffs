"""Generalized Dinkelbach method for fractional-linear expected metrics under calibration.

This module implements a general framework for optimizing any metric that can be expressed
as a ratio of two affine (linear + constant) functions of confusion matrix counts under
the calibration assumption. This includes F-beta, precision, recall, Jaccard/IoU,
Tversky index, accuracy, and specificity.
...
class FractionalLinearCoeffs:
    ...
    # Denominator coefficients: beta_0 + beta⋅[TP,TN,FP,FN] (must be > 0 for feasible selections)
    beta_tp: float = 0.0
    beta_tn: float = 0.0
    beta_fp: float = 0.0
    beta_fn: float = 0.0
...
def coeffs_for_metric(
    ...
    *,
    ...
    >>> print(f"Precision = {coeffs.alpha_tp}*TP / ({coeffs.beta_tp}*TP + {coeffs.beta_fp}*FP)")
    ...
    if m in {"jaccard", "iou", "intersection_over_union"}:
        # Jaccard = TP / (TP + FP + FN)
        return FractionalLinearCoeffs(alpha_tp=1.0, beta_tp=1.0, beta_fp=1.0, beta_fn=1.0)
    ...
    if m in {"f1"}:
        # F1 = 2*TP / (2*TP + FN + FP)
        return FractionalLinearCoeffs(alpha_tp=2.0, beta_tp=2.0, beta_fn=1.0, beta_fp=1.0)
    
    if m in {"f2"}:
        # F2 = 5*TP / (5*TP + 4*FN + FP)
        return FractionalLinearCoeffs(alpha_tp=5.0, beta_tp=5.0, beta_fn=4.0, beta_fp=1.0)
    
    if m in {"tversky"}:
        # Tversky(alpha,beta) = TP / (TP + alpha*FP + beta*FN)
        if tversky_alpha < 0 or tversky_beta < 0:
            raise ValueError(f"Tversky parameters must be non-negative, got alpha={tversky_alpha}, beta={tversky_beta}")
        return FractionalLinearCoeffs(
            alpha_tp=1.0,
            beta_tp=1.0,
...
def dinkelbach_expected_fractional_binary(
    ...
    are computed under the calibration assumption: E[TP] = sum(w_i * p_i) for i in S, etc.
    ...
    def sums_for_threshold(t: float, direction: Literal[">", "<"]) -> tuple[float, float, float]:
        """Compute sums over decision set S defined by threshold t and direction."""
        if direction == ">":
            # S = {i : p_i > t} or {i : p_i >= t} based on comparison
            idx = int(np.searchsorted(p_s, t, side="right" if comparison == ">" else "left"))
            # S is suffix [idx:]
            ...
            # S = {i : p_i < t} or {i : p_i <= t} based on comparison
            idx = int(np.searchsorted(p_s, t, side="left" if comparison == ">" else "right"))
            # S is prefix [:idx]
